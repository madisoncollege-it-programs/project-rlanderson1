...
#If there isn't an argument
    ...
    #Make a one by one instead 
            ...
            for entry in apache_log_entries:    
                ...
                #Incrementing count for IP address in dictionary
            ...
            #Write IP addresses with count >=5 to the analysis file
            apache_log_analysis = open('apache_analysis.txt', 'w')
            apache_log_analysis.write("IP Addresses with count >= 5:\n")
            for ip, count in apache_log_summary.items():
                if count >5:
                    apache_log_analysis.write(f"{ip}: {count}\n")
